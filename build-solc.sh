...
# --- Solidity >= 0.8.27 Termux Patches ---
# 1. CMakeLists.txt: Include fmtlib unconditionally (outside IGNORE_VENDORED_DEPENDENCIES)
# 2. libsolutil/CMakeLists.txt: Drop header-only targets (range-v3, nlohmann_json) & old Boost_SYSTEM_LIBRARY
if printf '0.8.27\n%s\n' "${VERSION}" | sort -V -C; then
    # Patch CMakeLists.txt
    if [ -f CMakeLists.txt ]; then
        sed -i '/if *(NOT IGNORE_VENDORED_DEPENDENCIES)/,/endif()/ { /include(fmtlib)/d }' CMakeLists.txt
        sed -i '/include(EthDependencies)/a include(fmtlib)' CMakeLists.txt
    fi

    # Patch libsolutil/CMakeLists.txt
    if [ -f libsolutil/CMakeLists.txt ]; then
        sed -i -E 's/target_link_libraries\(solutil PUBLIC .*\)/target_link_libraries(solutil PUBLIC Boost::boost Boost::filesystem fmt::fmt-header-only)/' libsolutil/CMakeLists.txt
    fi
fi
